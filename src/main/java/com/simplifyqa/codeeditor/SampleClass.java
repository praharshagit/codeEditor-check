...
import com.simplifyqa.abstraction.mainframe.driver.IQAMainframeDriver;
import com.simplifyqa.abstraction.mainframe.element.IQAMainframeElement;
import com.simplifyqa.pluginbase.codeeditor.annotations.AutoInjectMainFrameDriver;
...
public class SampleClass {
	...
	@AutoInjectMainFrameDriver
    private IQAMainframeDriver mainframeDriver;
	...
	@SyncAction(uniqueId = "sample-mainframe-fetch-and-validate-report", groupName = "Assertions", objectTemplate = @ObjectTemplate(name = TechnologyType.MAINFRAME, description = "Fetch a report from the mainframe, validate its contents, and log details"), objectRequired = false)
    public boolean fetchAndValidateMainframeReport(String appName, String reportLabel, String expectedText) {
        List<SubStep> substeps = new ArrayList<>();
        try {
            // Step 1: Launch Mainframe application
            boolean appLaunched = mainframeDriver.launchApplication(appName);
            SubStep substep1 = createSubstep("Launched mainframe application: " + appName, appLaunched, null);
            substeps.add(substep1);
            if (!appLaunched) {
                mainframeDriver.getExecutionLogReporter().reportSubSteps(substeps);
                return false;
            }
            // Step 2: Navigate to report section
            IQAMainframeElement reportLabelElement = mainframeDriver
                .findElement(com.simplifyqa.abstraction.mainframe.base.FindBy.label(reportLabel));
            boolean reportLabelClicked = reportLabelElement.click();
            SubStep substep2 = createSubstep("Clicked on report label: " + reportLabel, reportLabelClicked, null);
            substeps.add(substep2);
            if (!reportLabelClicked) {
                mainframeDriver.getExecutionLogReporter().reportSubSteps(substeps);
                return false;
            }
            // Step 3: Wait for the report content to load
            boolean textFound = mainframeDriver.waitForText(expectedText);
            SubStep substep3 = createSubstep("Waited for expected text in report: " + expectedText, textFound, null);
            substeps.add(substep3);
            if (!textFound) {
                mainframeDriver.getExecutionLogReporter().reportSubSteps(substeps);
                return false;
            }
            // Step 4: Validate the report content
            boolean textValidated = reportLabelElement.validateText(expectedText);
            SubStep substep4 = createSubstep("Validated presence of expected text: " + expectedText, textValidated, null);
            substeps.add(substep4);
            if (!textValidated) {
                mainframeDriver.getExecutionLogReporter().reportSubSteps(substeps);
                return false;
            }
            // Step 5: Capture a screenshot for evidence
            String screenshotPath = mainframeDriver.captureScreenshot();
            Map<String, String> screenshotData = formTestData(List.of("ScreenshotPath"), List.of(screenshotPath));
            SubStep substep5 = createSubstep("Captured screenshot of the validated report", true, screenshotData);
            substeps.add(substep5);
            // Report substeps and return true if all steps are successful
            mainframeDriver.getExecutionLogReporter().reportSubSteps(substeps);
            return true;
        } catch (Exception e) {
            log.severe("Error occurred in fetchAndValidateMainframeReport: " + e.getMessage());
            SubStep errorSubstep = createSubstep("Exception occurred during execution: " + e.getMessage(), false, null);
            substeps.add(errorSubstep);
            mainframeDriver.getExecutionLogReporter().reportSubSteps(substeps);
            return false;
	...
	@SyncAction(uniqueId = "sample-mainframe-login-and-fetch-data", groupName = "Assertions", objectTemplate = @ObjectTemplate(name = TechnologyType.MAINFRAME, description = "Login to mainframe, fetch data from a specific section, validate text presence"))
    public boolean loginAndFetchDataFromMainframe(String username, String password, String sectionLabel, String expectedText) {
        List<SubStep> substeps = new ArrayList<>();
        try {
            // Step 1: Enter username
            IQAMainframeElement usernameElement = mainframeDriver
                .findElement(com.simplifyqa.abstraction.mainframe.base.FindBy.label("Mainframe Username Field"));
            usernameElement.click();
            usernameElement.enterText(username);
            Map<String, String> usernameTestData = formTestData(List.of("username"), List.of(username));
            SubStep substep1 = createSubstep("Entered username into mainframe login field", true, usernameTestData);
            substeps.add(substep1);
            // Step 2: Enter password
            IQAMainframeElement passwordElement = mainframeDriver
                .findElement(com.simplifyqa.abstraction.mainframe.base.FindBy.label("Mainframe Password Field"));
            passwordElement.click();
            passwordElement.enterText(password);
            Map<String, String> passwordTestData = formTestData(List.of("password"), List.of(password));
            SubStep substep2 = createSubstep("Entered password into mainframe login field", true, passwordTestData);
            substeps.add(substep2);
            // Step 3: Click login button
            IQAMainframeElement loginButtonElement = mainframeDriver
                .findElement(com.simplifyqa.abstraction.mainframe.base.FindBy.label("Mainframe Login Button"));
            boolean loginClicked = loginButtonElement.click();
            SubStep substep3 = createSubstep("Clicked on mainframe login button", loginClicked, null);
            substeps.add(substep3);
            if (!loginClicked) {
                mainframeDriver.getExecutionLogReporter().reportSubSteps(substeps);
                return false;
            }
            // Step 4: Navigate to specific section
            IQAMainframeElement sectionElement = mainframeDriver
                .findElement(com.simplifyqa.abstraction.mainframe.base.FindBy.label(sectionLabel));
            boolean sectionClicked = sectionElement.click();
            SubStep substep4 = createSubstep("Navigated to section: " + sectionLabel, sectionClicked, null);
            substeps.add(substep4);
            if (!sectionClicked) {
                mainframeDriver.getExecutionLogReporter().reportSubSteps(substeps);
                return false;
            }
            // Step 5: Validate expected text presence
            boolean expectedTextFound = mainframeDriver.waitForText(expectedText);
            SubStep substep5 = createSubstep("Validated presence of expected text in section: " + expectedText, expectedTextFound, null);
            substeps.add(substep5);
            if (expectedTextFound) {
                String screenshotPath = mainframeDriver.captureScreenshot();
                Map<String, String> screenshotTestData = formTestData(List.of("ScreenshotPath"), List.of(screenshotPath));
                SubStep substep6 = createSubstep("Captured screenshot of validated section", true, screenshotTestData);
                substeps.add(substep6);
            }
            mainframeDriver.getExecutionLogReporter().reportSubSteps(substeps);
            return expectedTextFound;
        } catch (Exception e) {
            log.severe("Error occurred in loginAndFetchDataFromMainframe: " + e.getMessage());
            SubStep errorSubstep = createSubstep("Exception occurred during execution: " + e.getMessage(), false, null);
            substeps.add(errorSubstep);
            mainframeDriver.getExecutionLogReporter().reportSubSteps(substeps);
            return false;
    ...
    @SyncAction(uniqueId = "sample-reverse-string", groupName = "Generic", objectTemplate = @ObjectTemplate(name = TechnologyType.GENERIC, description = "This action reverses a given string"), objectRequired = false)
    public boolean newmethodSQA(String value) {
        if (value == null) {
            log.info("The provided string is null.");
            return false;
        }
